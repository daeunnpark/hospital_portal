    ...
    def authenticateUser(self):
        ...
        if(len(authenticate) == 1):
            ...
            cur.execute('SELECT ID FROM Person P WHERE Username = (%s) AND UserPassword = (%s)',
                        (self.lineEdit.text(), self.lineEdit_2.text()))
            ID = cur.fetchall()
            #cur.execute('SELECT Age FROM Patient P WHERE PatientID = (%d)', int(ID))
            #age = cur.fetchall()
            #cur.execute('SELECT Weight FROM Patient P WHERE PatientID = (%d)', int(ID))
            #weight = cur.fetchall()
            #cur.execute('SELECT Height FROM Patient P WHERE PatientID = (%d)', int(ID))
            #height = cur.fetchall()
            #cur.execute('SELECT SSN FROM Patient P WHERE PatientID = (%d)', int(ID))
            #ssn = cur.fetchall()
            #cur.execute('SELECT CreditCardNumber FROM Patient P WHERE PatientID = (%d)', int(ID))
            #creditCardNumber = cur.fetchall()
            #cur.execute('SELECT BillingAmount FROM Patient P WHERE PatientID = (%d)', int(ID))
            #billingAmount = cur.fetchall()
            #cur.execute('SELECT InsuranceNumber FROM Patient P WHERE PatientID = (%d)', int(ID))
            #insuranceNumber = cur.fetchall()
            #cur.execute('SELECT MedicationList FROM Patient P WHERE PatientID = (%d)', int(ID))
